...
public interface IPullToRefresh<T extends View> {
	 ...
	 * 演示下拉刷新的功能给用户，让他们意识到它的存在。
	 * 当用户第一次打开你的APP,这可能是有用的.
	 * 用户滚动到顶部/底部,动画会发生如果刷新视图（ListView，ScrollView)。
	 ...
	 * 返回触摸事件是否过滤或不过滤。
	 * 如果返回true，那么将只使用触摸事件。在此，在Y轴的差异大于X轴的差异。
	 * 这意味着视图将不会干扰一个水平滚动视图（如ViewPager）。
